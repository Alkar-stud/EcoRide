...
import { getToken, sendFetchRequest, sanitizeHtml } from '../script.js';
...
async function loadTickets() {
    try {
        const data = await sendFetchRequest(apiUrl + 'ecoride/employee/showValidations', getToken(), 'GET');
        lastData = data;
        ticketsList.innerHTML = '';
        if (!data || !Array.isArray(data) || data.length === 0) {
            ticketsList.innerHTML = '<div class="col-12 text-center text-muted">Aucun ticket à examiner.</div>';
            return;
        }
        ...
        document.querySelectorAll('.btn-take').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                openModal(lastData, ticketId, validationId);
            });
        });
    } catch (e) {
...
function openModal(data, ticketId, validationId) {
    ...
    if (validation.isClosed) {
        ...
        if (!document.getElementById('close-only-btn')) {
            ...
            } else if (saveBtn && saveBtn.parentElement) {
...
// Premier chargement des tickets
loadTickets();